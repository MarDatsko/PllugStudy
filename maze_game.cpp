...
void generateMaze(std::array<std::array<char, cMazeColumns>, cMazeRows> &prMaze)
{
	...
	//Генерує перешкоди на ігровому полі))
	srand((unsigned int)time(NULL));
	for (int column = 4; column < (cMazeColumns-1); column += 5)
	{
		int x = 2 + (rand() % (cMazeRows - 2));
		for (int row = 0; row < cMazeRows; row++)
		{
			prMaze[row][column] = cWallSymbol;
			if (prMaze[row][column] == prMaze[x][column])
			{
				prMaze[row][column] = cEmptySymbol;
			}
		}
	...
	// Генерує рандомне знаходження Ключа
	int aa, bb;
	do
	{
		aa = 1 + rand() % (cMazeRows - 1);
		bb = 1 + rand() % (cMazeColumns - 1);
		if (prMaze[aa][bb] != cWallSymbol)
		{
			prMaze[aa][bb] = Key;
			break;
		}
	...
	// Генерує рандомне знаходження Виходу
	do
	{
		X = rand() % cMazeRows;
		Y = rand() % cMazeColumns;
		if (prMaze[X][Y] != cWallSymbol)
		{
			prMaze[X][Y] = cExitSymbol;
			break;
		}
	} while (true);
}
...
bool moveCharacterAndCheckIfExitFound(std::array<std::array<char, cMazeColumns>, cMazeRows> &prMaze)
{
	...
	if (scanForChar(prMaze, cCharacterSymbol, charRow, charColumn))
	{
		...
		if (charMovedOnto == Key)
		{
			gameMessage("you find key!");
			rKey = true;
		}
	}
	else
